...
def dimension_reduction(X, k=0.95):
    """
    ...
    pct = pct.sum()
    print(
        "dataset of {0} dimensions has been reduced to {1} dimensions "
...
_data = pd.DataFrame(
    data=data, columns=["var_{0}".format(i) for i in range(data.shape[1])]
)
...
candidates = [
    ...
    GaussianProcessRegressor(
        kernel=RBF(length_scale=10.0, length_scale_bounds=(1e-5, 1e5)), alpha=1e-10
    ),
]

cv_pipes = [
    make_pipeline(StandardScaler(), PCA(n_components=8), estimator)
