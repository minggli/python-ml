...
df = (
    pd.DataFrame(data["data"], columns=data["feature_names"], dtype=np.float32)
    .join(pd.DataFrame(data["target"], columns=["species"], dtype=np.int32))
    .replace(target_dict)
)
...
x_train, x_test, y_train, y_test = model_selection.train_test_split(
    regressors, regressand, test_size=0.2
)

# Decision Tree classifier
reg = tree.DecisionTreeClassifier(
    max_depth=3, max_leaf_nodes=20, min_samples_leaf=15, random_state=2
)
