...
train_data, test_data = transform(data, target="Power", test_size=0.2)
train_batch, test_batch = make_queue(train_data), make_queue(test_data)

with sess:
    train_errors, train_r2s = train_nn(
        sess, x, y_, train_batch, test_batch, train_step, 500, r2, cost_func
    )
