...
def train_nn(sess, x, y_, train_batch, test_batch, optimiser, n_epochs, r2, loss):
    ...
    for epoch in range(n_epochs):
        ...
        for batch in train_batch_storage[epoch]:
            ...
            _, train_r2, train_error = sess.run(
                [optimiser, r2, loss], feed_dict={x: feat, y_: target}
            )
            step += 1
            if step % 5 == 0:
                batch = next(test_batch)
                feat, target = zip(*batch)
                test_r2, test_error = sess.run(
                    [r2, loss], feed_dict={x: feat, y_: target}
                )
                print(
                    "epoch: {0:<3} step: {1:<3} r2: {2:.4f} test error: {3:.6f}".format(
                        epoch, step, test_r2, test_error
                    )
                )
            print(
                "epoch: {0:<3} step: {1:<3} r2: {2:.4f} train error: {3:.6f}".format(
