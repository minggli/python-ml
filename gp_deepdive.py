...
assert np.allclose(
    radial_basis_function(a, b, lengthscale=1), rbf_kernel(a, b, gamma=0.5)
)
...
bool_mask = np.random.choice(a=[True, False], size=len(X_train), p=[0.5, 0.5])
...
# provable as follows
assert np.allclose(
    L_s.T @ (np.linalg.inv(L) @ y_train), K_s.T @ np.linalg.inv(K) @ y_train
)
...
# = K_ss - L_s.T * L_s
assert np.allclose(K_ss - K_s.T @ np.linalg.solve(K, K_s), K_ss - L_s.T @ L_s)
...
f_posterior = mu.reshape(-1, 1) + L_posterior @ np.random.normal(
    loc=0, scale=1, size=(N, S)
)
