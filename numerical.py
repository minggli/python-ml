...
# jit compiling
cov_inv_solution = partial(jit(covariance_matrix_inversion, forceobj=True), X, y)
cholesky_solution = partial(jit(cholesky_decomposition, forceobj=True), X, y)
qr_solution = partial(jit(qr_decomposition, forceobj=True), X, y)
svd_solution = partial(jit(svd_decomposition, forceobj=True), X, y)
print(timeit(cov_inv_solution, number=100))
print(timeit(cholesky_solution, number=100))
print(timeit(qr_solution, number=100))
print(timeit(svd_solution, number=100))
