...
def forward_select(data, target, alpha=0.05, display=True):
    ...
    while remaining and current_score == best_new_score:
        ...
        for candidate in remaining:
            X = sm.add_constant(
                data[selected_var + [candidate]]
            )  # inherit variables from last step and try new ones
            reg = sm.OLS(target, X).fit()
            score, p = (
                reg.rsquared,
                reg.pvalues[-1],
            )  # r2 (changeable) and two-tailed p value of the candidate
            scores_with_candidates.append((score, p, candidate))

        scores_with_candidates.sort(
            reverse=False
        )  # order variables by score in ascending
        ...
        try:
            best_new_score, best_candidate_p, best_candidate = (
                scores_with_candidates.pop()
            )
        except (
            IndexError
        ):  # when no candidate passes significance test at critical value
            disqualified_score, disqualified_p, disqualified_candidate = (
                disqualified_candidates.pop(0)
            )
            remaining.remove(
                disqualified_candidate
            )  # remove the worst disqualified candidate
...
def backward_select(data, target, alpha=0.05, display=True):
    ...
    while selected_var:
        ...
        scores_with_candidates.sort(
            reverse=False
        )  # order variables by p value in ascending order
        ...
        try:
            ...
            IndexError
        ):  # when no candidate fails the significance test at critical value
            selected_var = [i[2] for i in scores_with_candidates]
            break
