...
def generate_training_data(total_size, seq_length):
    ...
    assert total_size / seq_length > 1, (
        "total sequence must be larger than individual length."
    )
    sequence_whole = random.choices(population=[0, 1], weights=(0.5, 0.5), k=total_size)
...
X_train, X_test, y_train, y_test = model_selection.train_test_split(
    train, label, test_size=0.2, random_state=0
)

x = tf.placeholder(dtype=tf.float32, shape=[None, 1, STEP_SIZE], name="feature")
y_ = tf.placeholder(dtype=tf.uint8, shape=[None, N_CLASS], name="label")
init = tf.placeholder(dtype=tf.float32, shape=[None, STATE_SIZE])
...
def rnn_cell(rnn_input, state):
    ...
    with tf.variable_scope("rnn_cell", reuse=None):
        ...
        b_h = tf.get_variable(
            name="b_h", shape=[STATE_SIZE], initializer=tf.constant_initializer(0.0)
        )
...
for i in range(epoch):
    ...
    for j in range(no_of_batches):
        ...
        ptr += BATCH_SIZE
        sess.run(
            train_step, {x: inp, y_: out, init: np.zeros([inp.shape[0], STATE_SIZE])}
        )
    print("Epoch - ", str(i))
...
for j in range(no_of_batches):
    ...
    incorrect_rates.append(
        sess.run(
            error_rate,
            feed_dict={x: inp, y_: out, init: np.zeros([inp.shape[0], STATE_SIZE])},
        )
    )
